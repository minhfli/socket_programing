...
def handle_client(conn: socket.socket, addr):
    ...
    while connected:
        ...
        if step == 0:
            if msg.upper() != "HELLO":
                print(
                    f"[{addr}] Protocol error at step {step}: expected 'HELLO', got '{msg.upper()}'"
                )
                break
            step += 1
            send(conn, addr, f"{a} {b}")
        elif step == 1:
            try:
                s = int(msg)
                if s != a + b:
                    send(conn, addr, f"Wrong answer!")
                else:
                    send(conn, addr, f"Correct!")
                step += 1
            except ValueError:
                print(
                    f"[{addr}] Protocol error at step {step}: expected an integer, got '{msg}'"
                )
                break
        else:
            print(f"[{addr}] Protocol error: unexpected step {step}")
            break
